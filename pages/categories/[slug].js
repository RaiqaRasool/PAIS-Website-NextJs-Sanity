...
const ArchiveBlogs = ({blogs,categories}) => {
    const [blogSearchArchive,setBlogSearchArchive]=useState(blogs);
            ...
            <div className="section categories">
                ...
                <ul className="categories-list">
                {
                    categories?.map(category=>(
                        <li className="category"><a href={`/categories/${category.slug.current}`}>{category.title}</a></li>
                    ))
                }
                </ul>
            </div>
...
export default ArchiveBlogs;
  ...
  export const getServerSideProps=async({params:{slug}})=>{
    ...
    const query_blog=`*[_type=='post' && '${slug}' in categories[]->slug.current]{
            title,
            body,
            slug,
            mainImage,
    ...
    const blogs=await client.fetch(query_blog);
    return {
        props:{
            blogs,
            categories      
